...
<style>
    ...
    .access-card::before {
        ...
        height: 5px;
        background: var(--card-color, #2E74B5);
    }
    .access-card:hover {
        transform: translateY(-6px);
        box-shadow: 0 24px 64px rgba(0,0,0,0.12);
        border-color: var(--card-color, #2E74B5);
    }
    .access-icono {
        font-size: 44px;
        margin-bottom: 16px;
    ...
    .access-cta {
        ...
        text-transform: uppercase;
        color: var(--card-color, #2E74B5);
        border-bottom: 2px solid var(--card-color, #2E74B5);
        padding-bottom: 2px;
    }
    ...
    /* ocultar decoración de Streamlit en los botones de página */
    .stButton > button {
        border: none;
...
<div class="portal-header">
    <img src="data:image/jpeg;base64,{LOGO_B64}" style="height:52px; border-radius:6px;" />
    <div class="portal-header-text">
        <p class="portal-ministerio-label">Ministerio para la Transformación Digital y de la Función Pública</p>
        <p class="portal-titulo">Portal de Análisis · SGAMET</p>
        <p class="portal-subtitulo">S.G. de Análisis del Mercado y Evolución Tecnológica</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
st.markdown('<p class="seccion-titulo">Selecciona una herramienta</p>', unsafe_allow_html=True)
...
with col1:
    ...
    <div class="access-card" style="--card-color: #1F3864;">
        ...
        <p class="access-desc">Colección de estudios sectoriales sobre el mercado de las telecomunicaciones e infraestructuras digitales en España: fondos de inversión, satélites, cable submarino, torreros, fibercos, centros de datos e inteligencia artificial.</p>
        <span class="access-cta">Acceder →</span>
    </div>
    """, unsafe_allow_html=True)

    if st.button("📚 Ir a Monográficos", key="btn_mono", use_container_width=True):
        st.switch_page("pages/1_📚_Monograficos.py")
...
with col2:
    ...
    <div class="access-card" style="--card-color: #375623;">
        ...
        <p class="access-desc">Dashboard interactivo con las principales magnitudes económicas y de mercado de las operadoras de telecomunicaciones en España. Acceso verificado mediante contraseña.</p>
        <span class="access-cta">Acceder →</span>
    </div>
    """, unsafe_allow_html=True)

    if st.button("📊 Ir a Operadoras", key="btn_oper", use_container_width=True):
        st.switch_page("pages/2_📊_Operadoras.py")

st.markdown('</div>', unsafe_allow_html=True)

st.markdown("""
<p class="portal-pie">
    S.G. de Análisis del Mercado y Evolución Tecnológica · SETELECO · 2026
</p>
""", unsafe_allow_html=True)
